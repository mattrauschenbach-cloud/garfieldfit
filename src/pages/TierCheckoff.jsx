 ...
 *  - tier_checkoffs/<uid>_<tier> {
 *        uid, tier, completed: { [standardId]: true|false }, updatedAt
 *    }
 */
export default function TierCheckoff() {
  ...
  const toggleStandard = useCallback(async (sid) => {
    ...
    setSaving(true)
    try {
      // set (merge) so doc is created as needed
      await setDoc(ref, {
        uid: memberId,
        tier,
        completed: next,
  ...
  return (
    <section className="stack" style={{ gap: 16 }}>
      ...
      <div className="muted" style={{ fontSize:12 }}>
        Stored in <code>tier_checkoffs/&lt;uid&gt;_&lt;tier&gt;</code>. Public pages can read progress per tier.
      </div>
    </section>
  )
}
